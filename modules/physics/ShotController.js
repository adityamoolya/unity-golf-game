...
export class ShotController {
  ...
  executePutterShot(direction, clubDistance, powerFactor) {
    ...
    this.physics.putterShotDuration = 0.5;
  ...
  executeNormalShot(direction, clubDistance, powerFactor) {
    ...
    switch (this.gameState.selectedClub) {
      case 'driver':
        launchAngle = Math.PI / 12; 
        speedFactor = 1.0;
        break;
      case 'wood3':
        launchAngle = Math.PI / 10; 
        speedFactor = 0.95;
        break;
      case 'hybrid':
        launchAngle = Math.PI / 9; 
        speedFactor = 0.92;
        break;
      case 'iron2':
        launchAngle = Math.PI / 8.3; 
        speedFactor = 0.9;
        break;
      case 'iron3':
  ...
  updateTrajectoryArc() {
    // Skip updating if disabled by difficulty or if ball is moving
    const cfg = this.gameState && this.gameState.courseConfig;
    if ((cfg && cfg.disableTrajectory) || this.ballPhysics.gameState.ballInMotion) return;
  ...
  calculateClubVelocity(direction, clubDistance) {
    ...
    switch (this.gameState.selectedClub) {
      case 'driver':
        launchAngle = Math.PI / 12; // ~18° - flatter for driver
        break;
      case 'wood3':
        launchAngle = Math.PI / 10; // ~20°
        break;
      case 'hybrid':
        launchAngle = Math.PI / 9; // ~21°
        break;
      case 'iron2':
      case 'iron3':
        launchAngle = Math.PI / 8; // ~22.5°
