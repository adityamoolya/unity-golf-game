...
export class PhysicsEngine {
  constructor(ballPhysics) {
    ...
    // Physics constants (aligned with spec)
    this.gravity = 9.81;
    // Air resistance scalar (Cd proxy). Keep small since engine integrates simply
    this.airResistance = 0.004; // maps roughly to Cd 0.25–0.35 in our simple model
    // Surface frictions
    this.greenFriction = 0.2;   // Green: 0.2
    this.fairwayFriction = 0.25;  // Fairway: 0.25
    this.standardRoughFriction = 0.4; // Rough: 0.4
    this.outerRoughFriction = 0.9;    // Deep rough (kept high)
    this.sandFriction = 0.8;     // Sand: 0.8, heavy damping handled below
    
    // Bounce parameters
    // Coefficient of restitution ≈ 0.83
    this.bounceEnergyRetention = 0.83;
    this.maxBounces = 5;
    this.bounceCount = 0;
  ...
  handleBounce(ballLie, isPutter, verticalVelocity) {
    ...
    if (ballLie.inSand) {
      // Heavy damping in sand: drastically reduce bounce and horizontal
      progressiveLoss = this.bounceEnergyRetention * 0.05 * Math.pow(0.2, this.bounceCount);
      this.bounceCount += 2;
    } 
    ...
    let frictionFactor;
    if (ballLie.inSand) {
      frictionFactor = 0.85; // heavy damping on impact
    } 
    else if (ballLie.inOuterRough) {
      frictionFactor = 0.7;
    } 
  ...
  handleRoll(ballLie, terrainHeight) {
    ...
    if (ballLie.onGreen) {
      ...
      // Apply additional heavy drag in sand
      this.ballVelocity.x *= 0.5; // 50% reduction
      this.ballVelocity.z *= 0.5; // 50% reduction
