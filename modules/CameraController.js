...
export class CameraController {
  ...
  updateCameraModeButton() {
    ...
    positionCameraBehindBall(position) {
      // Clamp/sanitize position to avoid NaNs or infinities causing camera to fly away
      if (!isFinite(position.x) || !isFinite(position.y) || !isFinite(position.z)) {
        console.warn('Invalid ball position for camera reset, using last known position');
        position = this.lastBallPosition.clone();
      }
      ...
      // Clamp camera within course bounds
      const maxDist = this.gameState && this.gameState.courseConfig && this.gameState.courseConfig.groundSize ? this.gameState.courseConfig.groundSize * 0.45 : 180;
      cameraPosition.x = THREE.MathUtils.clamp(cameraPosition.x, -maxDist, maxDist);
      cameraPosition.z = THREE.MathUtils.clamp(cameraPosition.z, -maxDist, maxDist);
    ...
    updateCameraDuringFlight(position, prevPosition, velocity) {
      // Validate inputs to prevent erratic behavior
      if (!position || !prevPosition || !isFinite(position.x) || !isFinite(prevPosition.x)) {
        return;
      }
      ...
      if (movementDirection.length() > 0.01) {
        switch (this.gameState.cameraMode) {
          case 'follow':
            ...
            // Compute desired position and clamp to bounds
            ...
            const maxDist = this.gameState && this.gameState.courseConfig && this.gameState.courseConfig.groundSize ? this.gameState.courseConfig.groundSize * 0.45 : 180;
            desired.x = THREE.MathUtils.clamp(desired.x, -maxDist, maxDist);
            desired.z = THREE.MathUtils.clamp(desired.z, -maxDist, maxDist);
            // Smooth move to desired to avoid sudden jumps
  ...
  // Dispose controller to prevent ghost updates when switching levels
  dispose() {
