...
class GolfGame {
  ...
  initializeAfterSplash() {
    ...
    // Build difficulty-based overrides
    const difficulty = window.selectedDifficulty || 'easy';
    const overrides = (() => {
      if (difficulty === 'easy') {
        return { fairwayWidth: 40, roughWidth: 40, treeDensity: 40, heightVariation: 6 };
      }
      if (difficulty === 'hard') {
        // Level 3: Hard — keep trajectory disabled but ease terrain a bit
        return { fairwayWidth: 28, roughWidth: 70, treeDensity: 95, heightVariation: 16, disableTrajectory: true };
      }
      // Level 2: Medium — make fairer and remove wind gusts
      return { fairwayWidth: 32, roughWidth: 55, treeDensity: 70, heightVariation: 12 };
    })();

    // Merge selected hole with difficulty overrides
    const baseConfig = this.holeSelector.selectedHoleConfig || {};
    this.courseConfig = new CourseConfig({ ...baseConfig, ...overrides, difficulty });
    ...
    // Respect difficulty-based trajectory toggle
    if (!this.courseConfig.disableTrajectory) {
      this.ballPhysics.updateTrajectoryArc();
    } else {
      if (this.ballPhysics.trajectoryLine) this.ballPhysics.trajectoryLine.visible = false;
    }
  ...
  cleanup() {
    // Dispose camera controller loop
...
// Do not auto-start the game; splash will instantiate GolfGame when a level is chosen
...
// Expose constructor globally for splash helpers
// Ensures window.GolfGame is available when called from inline scripts
